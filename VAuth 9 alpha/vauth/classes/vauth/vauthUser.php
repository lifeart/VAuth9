	...
	class vauthUserModel {
		...
		private function getUserById($id=false) {	
			...
			if (is_numeric($id) && $id > 0) {
				$resp = array();
				$resp->id= $id;
				$dUserinfo = $this->load_table(USERPREFIX."_users", "*", "user_id = '$id'");
				if (!$dUserinfo) {
					$eParams = array();
					$eParams[] = $id;	
					newError($className,$functionName,'dUserInfo',$eParams);
				} else $resp->dle=$dUserinfo;
				$vUserinfo = $this->load_table(USERPREFIX."_vauth_users","*", "user_id = '$id'");
				if (!$vUserinfo) {
					$eParams = array();
					$eParams[] = $id;	
					newError($className,$functionName,'vUserInfo',$eParams);
				} else $resp->vauth=$vUserinfo;
				$vUserNetworks = $this->load_table(USERPREFIX."_vauth_user_networks","*", "user_id = '$id'",true);
				if (!$vUserNetworks) {
					$eParams = array();
					$eParams[] = $id;	
		...
		private function setDle ($id,$array) {
			...
			if (is_numeric($id) && $id>0) {
				...
				foreach ($array as $key=>$value) {
					$uResult[$key] = $this->db->query( "UPDATE " . PREFIX . "_users SET {$key}='{$value}' WHERE user_id='{$id}'" );
				}
				return $uResult;
			} else {
		...
		private function setVauth ($id,$array) {
			...
			if (is_numeric($id) && $id>0) {
				$uResult = array();	
				foreach ($array as $key=>$value) {
					$uResult[$key] = $this->db->query( "UPDATE " . PREFIX . "_vauth_users SET {$key}='{$value}' WHERE user_id='{$id}'" );
				}
				return $uResult;
			} else return false;
		}
		private function setNetwork ($id,$array) {
			...
			if (is_numeric($id) && $id>0) {
				...
				if (isset($array->id)) {
					...
					foreach ($array as $key=>$value) {
						try {
							$uResult[$key] = $this->db->query( "UPDATE " . PREFIX . "_vauth_user_networks SET {$key}='{$value}' WHERE id='{$array->id}'" );
						} catch (Exception $e) {
							$uResult[$key] = $e;
						}
					}
					...
					$this->db->query( "insert into " . USERPREFIX . "_vauth_user_networks (user_id) VALUES ('$id')" );
		...
		private function removeVauthUser($id) {
			...
			if (is_numeric($id) && $id > 0) {
				$this->db->query("DELETE FROM `".PREFIX."_vauth_user` where user_id = '{$id}'");
				return true;
			} else return false;
		...
		private function removeVauthUserNetwork($data) {
			...
			if (isset($data->id)) {
				if (is_numeric($data->id)) {
					$res->id=$this->db->query("DELETE FROM `".PREFIX."_vauth_user_networks` where id = '{$data->id}'");
				} elseif ($data->id = '*' && isset($data->user_id)) {
					$res->id=$this->db->query("DELETE FROM `".PREFIX."_vauth_user_networks` where user_id = '{$data->user_id}'");
				} else return false;
				return $res->id;
		...
		private function getUserByNetworkNameAndId($network=false,$id=false) {
			...
			if ($network!=false && $id!=false) {
				...
				$vUser = $this->load_table(USERPREFIX."_vauth_user_networks","*", "network = '$network' and id='$id'");
		...
		private function getUserFriendsByUidsAndNetwork($array,$network=false) {
			...
			$result = $this->load_table(USERPREFIX."_vauth_user_networks", "user_id", "uid in ({$userlist}) and network = '{$network}'",true);
			
			if ($result) {	
				$friends = array();
				foreach ($result as $value) {
					$friend = load_table(USERPREFIX."_users", "user_id,name,foto,fullname", "user_id = '{$value['user_id']}'");
		...
		function auth($network=false,$id=false) {
